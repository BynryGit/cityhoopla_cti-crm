...
def text_search(request):
    ...
    return render(request, 'Admin/text-search.html',data)


@csrf_exempt
def advert_search(request):
    print '------------in search----------',request.POST.get('advert_keyword')
    #pdb.set_trace()
    data = {}
    advert_list=[]
    advert_title = ''
    ...
    try:
        if request.method == "POST":
            if request.POST.get('advert_keyword'):
                text = request.POST.get('advert_keyword')
                try :
                    # advert_obj = Advert.objects.get(advert_name__icontains=text)
                    # ix=request.POST.get('search1')
                    # with ix.searcher() as searcher:
                    #     query = QueryParser.parse(u'ix')
                    #     results = searcher.Advert(query)
                    #     for result in results:
                    #         print result
                    advert_obj = Advert.objects.filter(keywords__icontains=text)
                    print '-------keywords------',advert_obj
                    if advert_obj:
                        for advert_title in advert_obj:
                            n=n+1
                            advert_title1 = advert_title.advert_id
                            advert_name = advert_title.advert_name
                            advert_area = advert_title.area
                            category = advert_title.category_id.category_name
                            advert_sub_obj = AdvertSubscriptionMap.objects.get(advert_id=advert_title1)
                            end_date = advert_sub_obj.business_id.end_date
                            edit = '<a style="text-align: center;letter-spacing: 5px;width:40%;" href="/edit-advert/?advert_id=' + str(advert_title1) + '"<i class="fa fa-pencil "></i> ' + '</a>'

                            advert_data={
                                'advert_title':advert_title1,
                        ...
                        advert_list.append(advert_data)
                    data = {'advert_list': advert_list, 'success':'true'}
                except Exception,e:
                    print e
                    data = {'advert_list': "No Result Found", 'success':'false'}
                data = {'advert_list': advert_list, 'success':'true'}
            else:
                data = {'message': "No Result Found", 'success':'false'}
...
def save_advert(request):
    ...
    try:
        if request.method == "POST":
            ...
            print '--------advert_keywords---------',request.POST.get('advert_keywords')
            advert_obj = Advert(
            ...
            try:
                ...
                try:
                    ...
                    if (premium_service_list):
                        print '------------after premium servuice------------',request.POST.get('advert_keywords')
...
def edit_advert(request):
    ...
    try:
        ...
        print 'keywords: ', advert_obj.keywords
        ...
        data = {'username': request.session['login_user'], 'keywords':keywords,'service_rate_card_list': service_rate_card_list,
...
def update_advert(request):
    ...
    try:
        if request.method == "POST":
            ...
            # except:
            #     advert_video = 0
            print '-------------advert----------',request.POST.get('advert_keywords')
